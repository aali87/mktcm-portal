...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Free printables claim origin:', origin);

    // Check if user is authenticated
    const session = await getServerSession(authOptions);
    if (!session?.user?.email) {
      return NextResponse.redirect(
        new URL('/auth/login?redirect=/programs/free-printables', origin)
      );
    }
    ...
    // Get the Free Printables product
    ...
    if (existingPurchase) {
      ...
      return NextResponse.redirect(
        new URL('/dashboard/programs/free-printables', origin)
      );
    }
    ...
    console.log('Free printables access granted to user:', user.email);

    // Redirect to the printables dashboard
    return NextResponse.redirect(
      new URL('/dashboard/programs/free-printables', origin)
    );
  } catch (error) {
    console.error('Free printables claim error:', error);
